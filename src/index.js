...
const Block = (tag, context) => {
  const exposedProps = context.props || {}
  ...
  class Connect extends React.Component {
    ...
    computeDataProps() {
      ...
      return mappers.reduce((prev, mapper) => {
        ...
        return { ...prev, ...mapper.data(state, props) }
      }, {})
    }

    runBindMethod(method) {
      const { state, props } = this
      return method(state, props)
    }
    ...
    render() {
      const { renderedElement, haveOwnPropsChanged } = this
      this.haveOwnPropsChanged = false

      const haveDataPropsChanged = this.updateDataProps()

      if (!haveOwnPropsChanged && !haveDataPropsChanged && renderedElement) {
